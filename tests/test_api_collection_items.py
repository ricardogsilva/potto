...
def test_api_collection_item_list_body(
    ...
    response = webapp_test_client_as_admin.get(f"/api/collections/{obs_feature_collection.resource_identifier}/items")
    assert response.status_code == 200
    payload = response.json()
    assert payload["type"] == "FeatureCollection"
    features = payload["features"]
    assert len(features) == len(obs_feature_collection.providers["feature"]["config"]["data"]["features"])
...
def test_api_collection_item_list_links(
    ...
    assert self_link["href"].endswith(f"/api/collections/{obs_feature_collection.resource_identifier}/items")
    collection_link = [li for li in links if li["rel"] == "collection"][0]
    assert collection_link["type"] == "application/json"
    assert collection_link["href"].endswith(f"/api/collections/{obs_feature_collection.resource_identifier}")
...
def test_api_collection_item_get_body(
    ...
    first_feature = obs_feature_collection.providers["feature"]["config"]["data"]["features"][0]
